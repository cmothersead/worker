...
export async function laneToLaneConsReports(browser, data, limboFlag = false, username, password) {
	...
	const old = readdirSync(
		'C:/Users/5260673/OneDrive - MyFedEx/Communication/Control Room Files/'
	).filter((fileName) =>
		fileName.includes(
	...
	for (const oldFile of old) {
		...
		unlinkSync(`C:/Users/5260673/OneDrive - MyFedEx/Communication/Control Room Files/${oldFile}`)
	}
	...
	if (flightNumbers) {
		...
		await startDateSelect.selectOption(
		...
		await endDateSelect.selectOption(
			today.toLocaleDateString('en-us', {
				month: 'long',
				day: 'numeric',
				year: 'numeric'
		...
		consNumbers = indexes
			.map((index) => consNumberValues[index])
			.filter((consNumber) => !!consNumber)
		const fNV = flightNumberValues.sort()
	...
	const links = consNumbers.map((number) =>
		page.locator(
			`a[href="/consreport/reportResultAction.do?method=exportCSV&consNumber=${number}"]`
		)
	)
	console.log('Opening CONS Report')
	await dropdown.selectOption('ursalane')
	await selectReportButton.click()
	await consNumbersField.fill(consNumbers.join('\n'))
	await nestedCheckbox.click()
	await queryReportButton.click()
	console.log('Query processing...')
	if (today.getDay() != 1 && limboFlag) await limbo(browser)
	await page.bringToFront()
	await links[0].waitFor({ timeout: 480000 })
	const downloadOperations = []
	page.on('download', (download) => downloadOperations.push(download))
	for (const link of links) {
		await link.click()
	}
	await Promise.all(downloadOperations)
	const downloadPaths = downloadOperations.map(async (download) => await download.path())

	console.log('Query complete')

	const root = 'C:/Users/5260673/OneDrive - MyFedEx/Lane to Lanes'
	...
	for (const fileName of downloadPaths) {
		const source = new AdmZip('C:/Users/5260673/OneDrive - MyFedEx/Documents/lanetolane.xlsx')
		const sheet = source.getEntry('xl/worksheets/sheet5.xml')
		console.log('Processing...')
		let flight = ''
		const lines = readFileSync(await fileName)
			.toString()
			.split('\n')
		...
		source.writeZip(
			`C:/Users/5260673/OneDrive - MyFedEx/Communication/Control Room Files/F${flight} ${
				dests[flight]
			} ${today.toLocaleDateString('en-us', {
				month: '2-digit'
...
export async function limbo(browser, customDate) {
	const page = await browser.newPage()
	await page.goto(
	...
	await download.saveAs('C:/Users/5260673/Downloads/' + download.suggestedFilename())
	await page.close()
	console.log('LIMBO data downloaded.')

	const limboDataFileName = readdirSync('C:/Users/5260673/Downloads/').filter(
		(fileName) =>
			fileName.match(/LIMBO - 57 columns.*\.csv/) &&
			Date.now() - statSync(`C:/Users/5260673/Downloads/${fileName}`).mtimeMs < 60000
	)[0]
	const limboData = readFileSync(`C:/Users/5260673/Downloads/${limboDataFileName}`)
		.toString()
		.replace(/\0/g, '')
		.split('\r\n')
		.map((line) => line.split('\t'))
	...
	const blank = new AdmZip('blank.xlsx')
	...
	console.log(limboResearchFilePath)
	console.log('LIMBO research downloaded.')

	const researchData = readFileSync(limboResearchFilePath).toString().replace(/\0/g, '')
	const researchOutput = parse(researchData, { delimiter: '\t' })
